...
#get classNum and save to ClsLabel.text
labelsList=train_df.Label.unique()
classNum=len(labelsList)
np.savetxt('ClsLabel.txt', labelsList, delimiter=',',fmt='%s')
...
def get_model(model):
    ...
    kwargs = {'input_shape': (imageSize, imageSize, 3),
              'include_top': False,
              'weights': 'imagenet',
              'pooling': 'avg'}
    ...
    x = tf.keras.layers.Dense(imageSize, activation='relu')(pretrained_model.output)
    x = tf.keras.layers.Dense(imageSize, activation='relu')(x)

    outputs = tf.keras.layers.Dense(classNum, activation='softmax')(x)
...
print("train images nums:", len(train_images))
print("val images nums:", len(val_images))
print("test images nums:", len(test_images))
print('\n')
...
#
#     model.save('./models/Mobilenetv3small_20220318_01.h5')
#
# for name, model in models.items():
...
model.save('./models/newest_PEseg.h5')
